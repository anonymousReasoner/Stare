...
public class CompressedTableau {
       ...
       ConceptLabel indLabel = new ConceptLabel();
        ...
        for (Startype s : l.getSstar()) {
            for (Triple t : s.getTriples()) {
                    for (Startype o : l.getSstar()) {
                        if (t.getRay().getTip().equals(o.getCore())) {
    ...
    public static Set<Startype> saturateLK(Startype s_1, Startype s_2, Layer l,  CompressedTableau ct,ReasoningData rd, MatchingFn mf, OWLOntology ontology) {
        Set<Startype> addedSetOfStartype = new HashSet<>();
